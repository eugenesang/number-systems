...
// 1. this function makes sure that no member of the vector exceeds the specified maximum value,
...
// 3. this function helps clean a vector by removing unnecessary 0's

vector<int> clean_vec(vector<int> a)
{
    vector<int> ret;
    vector<int> zero_carrier;
    for (int i : a)
    {
        if (i == 0)
        {
            zero_carrier.push_back(i);
        }
        else
        {
            for (int o : zero_carrier)
            {
                ret.push_back(o);
            }
            ret.push_back(i);
...
// division --the most complicated to implement --
// first of all we need to find a list of all multiples of the divisor between zero and the maximum number in the system

vector<vector<int>> generate_multiples(vector<int> divisor, int max)
{
    vector<vector<int>> ret;
    for (int i = 0; i < max; i++)
        ret.push_back(times_vec(divisor, i));
    return ret;
}
...
1. if the value of remainder has occurred twice --recurring
...
{
    vector<int> ret;
    vector<vector<int>> multiples = generate_multiples(b, 10);
